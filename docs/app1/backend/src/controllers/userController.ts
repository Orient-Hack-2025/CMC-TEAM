...
const register = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(200).send(new ApiResponse(200, { User }, 'User Added Successfully'));
});
...
const login = asyncHandler(async (req: Request, res: Response) => {
  ...
  const accessToken = jwt.sign({ id: user.id, email: user.email, role: user.role }, access_token, {
    expiresIn: '1d',
  });
  ...
  res.status(200).send(new ApiResponse(200, { accessToken }, 'User Login Successfully'));
});

const logout = asyncHandler(async (req: Request, res: Response) => {
  res.clearCookie('refreshToken');
  res.status(200).send(new ApiResponse(200, { message: 'User Logout Successfully' }));
});
...
const refreshToken = asyncHandler(async (req: Request, res: Response) => {
  ...
  const accessToken = jwt.sign({ id: user.id, email: user.email }, access_token, {
    expiresIn: '1d',
  });
  req.headers.authorization = `Bearer ${accessToken}`; //Is That Will Added Authomatically
  res.status(200).send(new ApiResponse(200, { accessToken }, 'User Refresh Token Successfully'));
});

const getMatchingSchools = asyncHandler(async (req: Request, res: Response) => {
  const schools = await matchSchoolStudent(req, res);
  res.status(200).send(new ApiResponse(200, schools, 'Schools Matched Successfully'));
});
